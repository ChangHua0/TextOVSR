...
from ..components.discriminators.clip_sr import CLIP_IMG_ENCODER, NetC,CLIP_IMG_ENCODER_larger,NetC_larger
import torch
...
class RealBasicVSR(RealESRGAN):
    ...
    def train_step(self, data_batch, optimizer):
        ...
        # prompts = data_batch['meta'][0]['degrade_prompts']
        # # 合成text描述，测试网络结构是否能顺利运行
        # prompt = 'A man with a long beard and a hat is sitting on a chair.'
        # prompts = [prompt] * (n*t)
        # prompts
...
class RealNegVSR(RealESRGAN):
    ...
    def __init__(self,
        ...
        super().__init__(generator, discriminator, gan_loss, pixel_loss,
                         ...
                         test_cfg, pretrained)
        #syx
        self.neg_loss = build_loss(neg_loss) if neg_loss else None
        #syx
        self.cleaning_loss = build_loss(
            cleaning_loss) if cleaning_loss else None
    ...
    def train_step(self, data_batch, optimizer):
        ...
        #syx
        fake_g_output,fake_g_output_rot, fake_g_lq = self.generator(lq, return_lqs=True)
        #syx
        ...
        #syx
        fake_g_output_rot = fake_g_output_rot.view(-1, c, h, w)
        #syx
        ...
        if (self.step_counter % self.disc_steps == 0
            ...
            #syx
            if self.neg_loss:
                losses['neg_loss'] = self.neg_loss(fake_g_output_rot, fake_g_output)
            #syx
...
@MODELS.register_module()
class TextOVSR_stage1(RealESRGAN):
    """RealBasicVSR model for real-world video super-resolution.
    ...
    def train_step(self, data_batch, optimizer):
        ...
        #fake_g_output, fake_g_lq = self.generator(lq, return_lqs=True)
        prompts = data_batch['meta'][0]['degrade_prompts']
        captions = data_batch['meta'][0]['captions']

        
        #syx
        fake_g_output,fake_g_output_rot, fake_g_lq = self.generator(lq,prompts,captions, return_lqs=True)
        #syx
        ...
        if (self.step_counter % self.disc_steps == 0
            ...
            # # [新增] 全局梯度裁剪
            # torch_utils.clip_grad_norm_(self.generator.parameters(), max_norm=1.0)  # 使用1.0作为起始值
            # 只对清理模块进行梯度裁剪
            torch_utils.clip_grad_norm_(self.generator.image_cleaning.parameters(), max_norm=1.0)  # 使用1.0作为起始值
...
@MODELS.register_module()
class TextOVSR_stage2(RealESRGAN):
    """RealBasicVSR model for real-world video super-resolution.
    ...
    def __init__(self,
        ...
        self.cleaning_loss = build_loss(
            cleaning_loss) if cleaning_loss else None
        self.clipiqa_loss = build_loss(clipiqa_loss) if clipiqa_loss else None
    ...
    def train_step(self, data_batch, optimizer):
        ...
        #syx
        fake_g_output,fake_g_output_rot, fake_g_lq, sent_emb,words_embs = self.generator(lq,prompts,captions, return_lqs=True)
        #syx
        ...
        if (self.step_counter % self.disc_steps == 0
            ...
            # CLIPIQA_loss
            if self.clipiqa_loss:
                losses['loss_clipiqa'] = self.clipiqa_loss(fake_g_output)
            #syx
            ...
            # gan loss for generator
            if self.gan_loss:
                fake_g_pred = self.discriminator(fake_g_output,sent_emb)
                losses['loss_gan'] = self.gan_loss(
                    fake_g_pred, target_is_real=True, is_disc=False)
        ...
        if self.gan_loss:
            set_requires_grad(self.discriminator, True)
            # real
            real_d_pred = self.discriminator(gt_gan,sent_emb)
            loss_d_real = self.gan_loss(
                real_d_pred, target_is_real=True, is_disc=True)
            loss_d, log_vars_d = self.parse_losses(
                dict(loss_d_real=loss_d_real))
            ...
            # fake
            fake_d_pred = self.discriminator(fake_g_output.detach(),sent_emb)
            loss_d_fake = self.gan_loss(
                fake_d_pred, target_is_real=False, is_disc=True)
            loss_d, log_vars_d = self.parse_losses(
                dict(loss_d_fake=loss_d_fake))
