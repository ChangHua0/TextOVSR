...
from mmedit.models.backbones.sr_backbones.basicvsr_net import (
    BasicVSRNet, ResidualBlocksWithInputConv,TextOVSRNet)
...
#
@BACKBONES.register_module()
class Real_TextOVSRNet(nn.Module):
    """RealBasicVSR network structure for real-world video super-resolution.
    ...
    def __init__(self,
        ...
        # BasicVSR
        self.basicvsr = TextOVSRNet(mid_channels, num_propagation_blocks,
                                    spynet_pretrained)
        self.basicvsr.spynet.requires_grad_(False)

    def forward(self, lqs, prompts, captions, return_lqs=False):
        n, t, c, h, w = lqs.size()
        ...
        # Super-resolution (BasicVSR)
        if self.training:
            outputs, outputs_rot,sent_emb,words_embs = self.basicvsr(lqs, prompts, captions)

            if return_lqs:
                return outputs, outputs_rot, lqs, sent_emb,words_embs
            else:
                return outputs, outputs_rot
        else:
            outputs = self.basicvsr(lqs, prompts, captions)
